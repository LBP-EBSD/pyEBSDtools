...
class SpatialFieldGenerator:
    ...
    def _add_noise(self, eps_map: np.ndarray, noise_frac: float) -> np.ndarray:
        ...
        # Light spatial smoothing: 5-tap moving average along rows then cols
        from numpy import convolve
        k = np.ones(5) / 5.0
        for comp in range(6):
            for r in range(self.grid_rows):
                raw_noise[r, :, comp] = convolve(raw_noise[r, :, comp], k, mode='same')
            for c in range(self.grid_cols):
                raw_noise[:, c, comp] = convolve(raw_noise[:, c, comp], k, mode='same')
